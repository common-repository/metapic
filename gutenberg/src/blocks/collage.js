 ...
 */
registerBlockType('metapic/collage', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __('Metapic Collage'), // Block title.
	...
	edit: function ({attributes, className, setAttributes}) {
			...
			if (attributes.imageUrl) {
				...
				return (
					<div className="button-container">
						<Button
							onClick={() => openTagEditor(attributes, 'collage', setAttributes)}
							className="button button-large"
						>
							Click to create your collage
						</Button>
					</div>
				);
			}
		...
		if (!hasOpened && !attributes.imageUrl) {
			openTagEditor(attributes, 'collage', setAttributes);
			hasOpened = true;
		}

		return (
			getImageButton(openTagEditor)
		);
	},
