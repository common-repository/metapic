...
$deepLinkActive = $this->has_active_account() ? get_option( $autoName ) : get_site_option( $autoName );
$deepLinkPost = $this->is_edit_page( 'new' )
	? $deepLinkActive : get_post_meta($post->ID,
	'mtpc_deeplink_auto', true);
$deepLinkStatus = $deepLinkPost
	? __( 'Active', 'metapic' ) : __( 'Inactive',
	'metapic' );
?>
<div class="misc-pub-section mtpc-deeplinking">
	...
	<span id="deeplink-status-display">
		<span class="deeplink-status-text status-0" <?php if ($deepLinkPost) { ?>style="display: none;"<?php } ?>><?= __( 'Inactive',
				'metapic' ) ?></span>
		<span class="deeplink-status-text status-1" <?php if (!$deepLinkPost) { ?>style="display: none;"<?php } ?>><?= __( 'Active',
				'metapic' ) ?></span>
	</span>
	...
	<div class="hide-if-js" id="deeplink-status-select">
		<div class="deeplink-status-edit">
			...
			<label class="selectit" for="deeplink-status-check"><?= __( 'Activate auto linking',
					'metapic' ) ?></label>
		</div>
		<p>
